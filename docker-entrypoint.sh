...
if [ ! -f /opt/firecracker/rootfs/ubuntu.ext4 ]; then
    ...
    if [ -f /opt/firecracker/rootfs/ubuntu.ext4 ]; then
        ...
        echo "→ Installing Ubuntu base system with debootstrap (timeout: 30 minutes per attempt)..."
...
echo "→ Validating rootfs integrity..."
...
echo "✓ Rootfs found: $(ls -lh /opt/firecracker/rootfs/ubuntu.ext4 | awk '{print $5}') [VALID]"
echo "✓ Kernel found: $(ls -lh /opt/firecracker/kernel/vmlinux-6.12 | awk '{print $5}')"
